...
pushd %{_builddir}/%{_module_name}/mpic
cp *.so*       %{buildroot}/%{libpath}

cp libmpic.a          %{buildroot}/%{libpath}
popd

#devel files
pushd %{_builddir}/%{_module_name}/mpic
mkdir -p %{buildroot}/%{includepath}/mpic
mkdir -p %{buildroot}/%{includepath}/mpic/http
mkdir -p %{buildroot}/%{includepath}/mpic/httpc
mkdir -p %{buildroot}/%{includepath}/mpic/udp
cp -rf *.h        %{buildroot}/%{includepath}/mpic
cp -rf http/*.h   %{buildroot}/%{includepath}/mpic/http
cp -rf httpc/*.h  %{buildroot}/%{includepath}/mpic/httpc
cp -rf udp/*.h    %{buildroot}/%{includepath}/mpic/udp
